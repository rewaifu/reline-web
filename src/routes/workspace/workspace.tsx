...
import { COLUMN_KEYS, createColumnsLayout, type ColumnKey, type ResizableSide } from "~/hooks/use-columns-layout"
...
const Workspace: Component = () => {
  ...
  const { layout, resizing, visibleColumns, toggleHidden, startResize, moveResize, endResize, nudge, resetWidth } =
    createColumnsLayout()
  ...
  const flexKey = () => (hidden("middle") ? columns()[columns().length - 1]! : "middle")
  /** The divider left of `key` resizes the stack's neighbour — right column, or left when the stack is hidden. */
  const neighbourOf = (key: ColumnKey): ColumnKey => (key === "right" && !hidden("middle") ? "middle" : "left")
  const sideOf = (key: ColumnKey): ResizableSide => (neighbourOf(key) === "middle" ? "right" : "left")
  /** Columns are kept mounted (only hidden) — unmounting Kobalte tabs halts Solid's reactive graph. */
  const splitterActive = (key: ColumnKey) => !hidden(key) && columns().indexOf(key) > 0
        ...
        <For each={COLUMN_KEYS}>
          {(key, index) => (
            <>
              <Show when={index() > 0}>
                <div
                  class={[styles.splitter, !splitterActive(key) && styles.hidden]}
                  role="separator"
                  aria-orientation="vertical"
                  aria-label={`Изменить ширину: ${COLUMN_LABELS[neighbourOf(key)]}`}
                  tabindex="0"
              ...
              <div
                class={[styles.column, hidden(key) && styles.hidden, key === flexKey() && styles.flex]}
                style={
                  key === "middle"
                    ? undefined
                    : {
                        "--col-width": `${layout()[key]}px`,
                        "--col-min-width": `${key === "right" ? MIN_RIGHT_WIDTH : MIN_SIDE_WIDTH}px`,
                      }
                }
              >
                <Switch>
                  ...
                  <Match when={key === "middle"}>
                    <NodeCards selectedId={selectedId} onSelect={setSelectedId} />
                  </Match>
