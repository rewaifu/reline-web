...
export const convertFolderReaderToPure: ConvertToPureFunction = (nodes, index, preprocess) => {
  const node = nodes[index]
  const { unarchive, ...pureOptions } = v.parse(folderReaderOptionsSchema, node.options)
  if (unarchive) {
    preprocess.push({
  ...
  return [[{ type: PureNodeType.FOLDER_READER, options: pureOptions }], index + 1]
}
...
export const convertFolderReaderToStack: ConvertToStackFunction = (nodes, index, ctx) => {
  const node = nodes[index]
  const parsed = v.parse(readerImportSchema, node.options)
  const options = { ...parsed, unarchive: ctx.unarchivedPaths.has(parsed.path) }
  return [
    [
      {
        id: index,
