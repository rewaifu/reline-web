...
import type { NodeOptions, PureConfig, PureNode, PureNodeOptions, StackNode } from "~/types/node"
import { MODEL_POSTFIX, MODEL_PREFIX } from "~/constants"
import { convertHalftoneToStack, convertScreentoneToPure } from "~/lib/convert/halftone"
import { convertResizeToPure, convertResizeToStack } from "~/lib/convert/resize"
import { convertUpscaleToPure, convertUpscaleToStack } from "~/lib/convert/upscale"
import { DEFAULT_COLLAPSED } from "~/constants"
import { convertFolderReaderToPure, convertFolderReaderToStack } from "~/lib/convert/folder-reader"
...
/** Import context: names of the models downloaded by the preprocess section. */
...
const convertToStackMapper: Partial<Record<PureNodeType, ConvertToStackFunction>> = {
...
export const convertToPure = (nodes: StackNode[]): PureConfig => {
  ...
  for (let i = 0; i < nodes.length; ) {
    const [converted, nextIndex] = convertToPureMapper[nodes[i].type](nodes, i, preprocess)
...
const dedupeDownloads = (preprocess: PureNode[]): PureNode[] => {
  ...
  for (const node of preprocess) {
    ...
    const parents = new Set([...(existing.meta?.parents ?? []), ...(node.meta?.parents ?? [])])
...
const importContext = (preprocess: PureNode[] | undefined): StackImportContext => {
  const downloadedModels = new Set<string>()
  const unarchivedPaths = new Set<string>()
  for (const node of preprocess ?? []) {
    if (node.type === PureNodeType.DOWNLOAD && "name" in node.options) {
      downloadedModels.add(node.options.name)
    }
...
const convertPureList = (nodes: PureNode[], ctx: StackImportContext): StackNode[] => {
...
export const convertToStack = (pure: PureConfig | PureNode[]): StackNode[] => {
  if (Array.isArray(pure)) {
    ...
    return convertPureList(pure, { downloadedModels: new Set(), unarchivedPaths: new Set() })
  }
